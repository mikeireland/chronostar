...
class NaiveFit(object):
    ...
    def run_fit_parallel(self): # Not finished
        """
        Perform a fit (as described in Paper I) to a set of prepared data.
        ...
        while self.ncomps <= self.fit_pars['max_comp_count']:
            ...
            for i, target_comp in enumerate(prev_result['comps']):
                ...
                # Save fit pars for the i-th component split
                filename_pars_i = os.path.join(run_dir, 'local_fit_pars_%d.npy'%i)
                np.save(filename_pars_i, self.fit_pars)
                filenames_pars.append(filename_pars_i)

            # RUN EM ALGORITHM WITH EXTERNAL CODE


            # READ RESULTS
            for i, target_comp in enumerate(prev_result['comps']):
                result = None#self.run_em_unless_loadable(run_dir)
                all_results.append(result)
