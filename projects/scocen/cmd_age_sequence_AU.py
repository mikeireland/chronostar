...
for c2 in comps_to_plot:
    ...
    # CUT
    ax2.scatter(t['bp_rp_extinction_corrected'], t['Gmag_extinction_corrected'], s=3, c=colors[comp_ID], label=r'%s %.0f $\pm$ %.0f Myr'%(comp_ID, age, c['Crossing_time']))
    ...
    # Check if the lower part of U contains Chamaeleon stars that are 5Myr
    #~ if comp_ID=='U':
        #~ mask = t['b']>-5
        #~ t=t[mask]
        #~ print(len(t))
        ...
        #~ ax.scatter(t['bp_rp_extinction_corrected'], t['Gmag_extinction_corrected'], s=10, c=colors[comp_ID], label=r'%s %.1f $\pm$ %.1f Myr'%(comp_ID, age, c['Crossing_time']), edgecolor=edgecolor)
...
legend=ax.legend(handles, labels, markerscale=5, frameon=False, loc='center right', bbox_to_anchor=(0.43, 0.195), title='Kinematic ages', prop={'size': 8})
...
plt.savefig('cmd_li_AU_90percent_membership.pdf')
plt.show()
